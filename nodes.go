...
func (n *Node) Vzdump(ctx context.Context, options VirtualMachineBackupOptions) (task *Task, err error) {
	var upid UPID

	if err = n.client.Post(ctx, fmt.Sprintf("/nodes/%s/vzdump", n.Name), &options, &upid); err != nil {
		return nil, err
	}
